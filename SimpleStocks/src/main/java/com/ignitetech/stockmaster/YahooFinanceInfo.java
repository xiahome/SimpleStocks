...
public class YahooFinanceInfo {
  ...
  public static Stock getStockData(String ticker) {
    ...
    try {
      URL url = new URL(
          "http://finance.yahoo.com/d/quotes.csv?s=" + ticker + "&f=" + yPrice + yChange + yPrevClose + yOpen + yBid
              + yAsk + yOneYrTarget + yMarketCap + yTrailingPE + yForwardPE + yPegRatio + yPriceSales + yPriceBook
              + yEbitda + yTradeDate + yTradeTime + yExchange + yAvgDailyVolume + yVolume + yDaysRange + yYearRange
              + yName);
